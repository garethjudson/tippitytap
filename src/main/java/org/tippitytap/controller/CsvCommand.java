...
public class CsvCommand {
  ...
  private static final String[] csvOutputHeader =
      new String[] {
        "Started",
        "Finished",
        "DurationSecs",
  ...
  // In normal process I would limit the amount of exceptions I throw, and look to handle them where
  // they are thrown
  // I would usually like to wrap exceptions in a result object which specifies when processes
  // completed successfully vs with errors
  // this allows the caller to use the Error value for logic without having to use exception
  // handling--which is a general smell.
  // This can have some effects depending on framework, e.g. exceptions and hibernate transactions
  // when exceptions get thrown
  // in this case I don't have as much time as I would like to set this up. So...right now we're not
  ...
  public void ProcessCsvFile(String csvFileIn, String csvFileOut) throws IOException {
    try (CsvReader<NamedCsvRecord> csvInput =
      ...
      var trips = tripService.processTaps(csvInput.stream().map(this::tapFromCsvRow).toList());
  ...
  // I have not done this as, for the most part it will only be used here and this is just an
  // example
